import { ActivityLevel, Goal, MacroTargets, NutritionProfile, Sex } from './types';

/** Calorias a partir dos macros (regra 4-4-9). */
...
/** Ajuste calórico conforme o objetivo. */
...
/** True quando o perfil já tem os dados físicos necessários. */
export const hasPhysicalProfile = (u: NutritionProfile | null | undefined): boolean =>
  !!(u && u.sex && u.age && u.heightCm && u.weightKg && u.activityLevel && u.goal);

/** Orquestra TMB → GET → ajuste por objetivo → split de macros. */
export const computeTargets = (u: NutritionProfile): MacroTargets | null => {
  ...
  const get = tdee(bmr, u.activityLevel as ActivityLevel);
  const kcalTarget = calorieTargetForGoal(get, u.goal as Goal);
  return macroSplitForGoal({ kcalTarget, weightKg: u.weightKg!, goal: u.goal as Goal });
};

/** Resumo dos números intermediários — útil para exibir no perfil. */
export const computeProfileSummary = (u: NutritionProfile) => {
  if (!hasPhysicalProfile(u)) return null;
  const bmr = bmrMifflinStJeor({
    sex: u.sex as Sex,
    weightKg: u.weightKg!,
  ...
  const targets = macroSplitForGoal({
    kcalTarget: calorieTargetForGoal(get, u.goal as Goal),
    weightKg: u.weightKg!,
    goal: u.goal as Goal,
  });
