...
function TooltipContent({ active, payload, label, fmt }: { active?: boolean; payload?: { value: number }[]; label?: string; fmt?: (v: number) => string }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="vt-tooltip">
      <p className="text-[var(--text-soft)] text-[11px] mb-1">{label}</p>
      {payload.map((p, i) => (
        <p key={i} className="font-black">{fmt ? fmt(p.value) : p.value}</p>
      ))}
    </div>
  );
}

export default function BarChartSimple({ data, color = '#8B5CF6', color2, height = 140, formatValue }: Props) {
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -28 }} barGap={4}>
        ...
        <Tooltip content={<TooltipContent fmt={formatValue} />} cursor={{ fill: 'rgba(0,0,0,0.04)', radius: 8 }} />
